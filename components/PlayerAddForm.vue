        ...
        <form class="flex flex-col gap-5" @submit.prevent="onAddPlayer">
            <label for="player"
                ...
                <input id="player"
                       ...
                       required="true"
                       class="peer h-8 w-full border-none bg-transparent p-0 placeholder:text-transparent focus:border-transparent focus:outline-none focus:ring-0 sm:text-sm dark:text-white" />

                <span class="absolute start-3 top-3 -translate-y-1/2 text-xs text-gray-700 transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:text-sm peer-focus:top-3 peer-focus:text-xs dark:text-gray-200">
                    Ajouter un joueur
                </span>
            </label>
